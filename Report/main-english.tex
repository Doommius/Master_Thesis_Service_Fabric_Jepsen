...
The ACID Model of handling database transactions is considered monolithic by some, however they still serve a vital a critical function for handling critical systems that require atomicity, consistency across the entire dataset, reliability, and durability 
...
The acronym in Cap is defined by
\begin{itemize}
    \item Basically available indicates that the system does guarantee availability, in terms of the CAP theorem.
\item Soft state indicates that the state of the system may change over time, even without input. This is because of the eventual consistency model.
\item Eventual consistency indicates that the system will become consistent over time, given that the system doesn't receive input during that time.
\end{itemize}

However these definitions differ from the definitions in ACID, However they are quite important when it comes to distributed systems and their behavior. 
Consistency in ACID is define as constraints on the data. By the CAP consistency concept, ACID would follow Sequential consistency as defined by Lamport\cite{lamport1993how}: “the program behaves as if the memory accesses of all processes were interleaved and then executed sequentially.” while the consistency model in CAP is defined as Atomic Consistency (also called linearizability) is sequential plus a real-time constraint: “Unlike sequential consistency, linearizability implicitly assumes the notion of an observable global time across all processes. Operations are modeled by an interval which consists of the period of time between the invocation and response for the operation and each operation is assumed to take effect instantaneously at some point within this interval.” \cite{CSL-TR-95-685}

The concept of linearizability will be investigated deeper doing the presentation of Jepsen where this is a key way of investigating weather a system lives up to the guarantees of a given system.


\subsection{Consistency models.}

2 consistency models were mentioned already, Atomic Consistency and Sequential consistency, 2 more consistency models are defined, Causal+ Consistency and  Eventual Consistency
