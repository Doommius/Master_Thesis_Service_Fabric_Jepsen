...
Jepsens internal modules handle consistency between the clients and connections to the target system.
...
% Checker. PLDI '10, 2010.
    ...
    If the case of two nodes reporting each other failed, the conflict is resolved either by majority, i,e,.  the node that was reported first to the most Arbitrators leaves the ring, or a second option where the membership is healed if both nodes are healthy and can therefore stay. \\   \vspace{5mm}
...
\begin{enumerate}
    \item The testing suite is run in Azure using six nodes. Five for service fabric and 1 for the Jepsen host. Service Fabric is in charge of the cluster for Orchestration. a deployment configuration is provided with the source code,
...
Implementing the Jepsen test of Service Fabric was not the most seem less experience. 2 new programming languages, programming patterns, and frameworks were introduced. these new framworks led to many beginners' mistakes due to the lack of experience with .Net, Clojure, and the MVC design paradigm. 
\\ \vspace{5mm}
