...
\subsubsection{Example of violation}
Suppose transaction T1 modifies x by appending [2]. Transaction T2 then reads that x before T1 performs a COMMIT. If T1 then performs a ROLLBACK, T2 will have read x = [1,2], an object that was never committed and that may thus be considered to have never existed.\\
\vspace{5mm}
\tikzset{every picture/.style={line width=0.75pt}} %set default line width to 0.75pt        
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
%uncomment if require: \path (0,300); %set diagram left start at 0, and has height of 300
...
\vspace{5mm}
The reason for doing these tests is to ensure that a system behaves expectedly and to verify that they behave according to how they are presented and defined by their creators. If a system does not behave as expected, situations can occur where information is lost, and harm can be done either bodily, financially, or materially. \\ \vspace{5mm}
These situations occur as Databases often sacrifice consistency to gain performance or higher availability. In context, older style transactional databases might only allow for the execution of one transaction at a time where either the whole database or tables are locked by this transaction. Other implementations allow for transactions to execute concurrently to allow for higher performance and better scalability. However, these optimizations often have side effects due to either poor implementations, understanding, or simply intent to gain performance. These side effects might range from sometimes returning a stale value to the dirty reads and writes that allow for uncommitted data to persist. 

\section{Historically}

Historically, this was done using manually defined sets of transactions. They required careful consideration and were time-consuming to design, and often only able to check for simple patterns. These tests were run on a given system, checking if a proven invariant hold for a given consistency model or if an anomaly is present in concurrent execution; this was done by inserting two records,  x, and y, in two separate transactions and in two following transactions checking if we can observe x but not y, or y and not x, as this could show how the systems handle long forks and snapshot isolation, and more importantly if it supports them at all.

These tests are generally quite efficient but only check for specific patterns and therefore do not allow for the larger picture of where issues might be present. They are also highly limited in what anomalies they can check and which consistency models they can check. Furthermore, due to the hard-coded and highly custom design, they are implemented on a system-to-system basis with no interchangeability.

This also means that the scope of these testing projects usually is enormous and defined case by case with predefined transactions that only test a limited scope. The test might have been done previously but were very limited in test coverage, and most anomalies were are never detected,

\section{State of the art}

State of the art is Jepsen, or rather one of the state of the Art projects.

The Jepsen client is built up in a modular fashion such that the different modules needed for a test can be defined independently, and a given project might not require all modules. A Jepsen test suite generally consists of 5 modules. Database automation, client, generator, correctness checkers, and a fault introduction. On top of these modules, a test runner is required to connect the modules.

The goal of Jepsen was to have a framework that allows 

The more inner workings of Jepsen consist of a Host node and tentacles going connecting to the nodes in the test cluster to perform of\todo[author=Daniela]{preform of the tasks?} the tasks such as database automation and the introduction of faults.
...
\subsection{Database Automation}
 The Database or db module job handles starting, stopping, and interaction with the database program or service itself. Its main job is to spin up, prepare and configure a test target; in some instances, this might not be required, but it is best practice to do this as it allows for better repeatably of the tests. A just as important feature is interacting with the service ones it is running, killing workers, inducing crashes, or removing the entire node from the cluster.

\subsection{Client}
The Client is an interface from a map of commands to a connector or driver. It handles parsing \& validation of return values, exception and faults handling, and routing requests to the correct endpoint.

\subsection{Generator}
The generator's task is to generate the test map, which includes transactions and faults to allow for correctness checking. Our checker needs to be able to defer an ordered history from the responses from the target database.

\subsection{Correctness checker}
The checker module checks for anomalies in the results; this can be a host of symptoms, out-of-order operations, violation of isolation, corrupt data due to conflicts. This can either be done by writing custom checkers or using some of the functionality built into Jepsen.

\subsection{Fault inducer}
Nemesis is the module within Jepsen that induces faults; its task is to induce faults at random such as time skew, network partitions, crashes, increased package drop, increased latency, and other tools.




Jepsen supports quite a few different modules, but the most powerful checker is Elle, inferring isolation anomalies from Experimental Observations. \\
\vspace{5mm}
This works by inferring a dependency graph between client-side observations and the database version history. This is done by carefully selecting database objects and operations such that the database reads reveals information about the version history.\\
\vspace{5mm}
This means that Elle can reveal any anomaly and provide a concise explanation as to why a fault occurred with information of what conditions were present to cause this fault to occur. Using this information, we can say how a system behaves, what level of consistency and which behavior we will see, what promises the system makes, and how promises are reflected in reality.\\
\vspace{5mm}

And these things can be manifested in multiple ways; some cases are, as mentioned earlier in the paper, accepted as eventually consistency and performance is valued higher, or some applications were losing some data points are acceptable to allow for higher write throughput.\\
\vspace{5mm}

Moreover, the same goes for stale, dirty, non-repeatable, and phantom reads are not an issue. This could be on hyper-scale social networks where a user might see something that was deleted, or if the newest posts, comments, and likes are not distributed across all nodes, they will eventually. Nevertheless, this is a trade-off that allows for much higher write performance as the chance of a user making changes to the same page on two different nodes/clusters is minimal. The required performance drop required to facilitate the guarantee that all data is always up to date would mean that the system would not scale at the rate it is required; this also brings up another interesting topic, approximate programming, where a program does not always have to return the correct output. However, if we can see a magnitude speedup of a program and accept a faulty result rate of a few \%, then the cost-saving and capacity increase could very much be worth it. This could be in the case of Netflix and Amazon recommencing products and services, it does not have to be perfect, and maybe sometimes the result is wrong, but if it means we can serve a lot more customers before having to degrade services or maybe having it as a step in the service degradation levels to support as many users as possible. \\
\vspace{5mm}
However, a lot of this lies outside the scope of this thesis, but it is also a super exciting field that will probably see growth in the next decade.
...
%    https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-reliable-services-reliable-collections
    ...
    SF-Ring and consistent routing. 
...
\begin{itemize}
    ...
    \item Casandra last-write wins issues, %and an attempt at a solution via Atomic clocks
    \item Elastic search issues.
\end{itemize}
\todo[author=Mark]{add some more information to the above points}

These issues can have huge implications in the real world if the behavior is not well understood. Just here in Odense, MongoDB is used by the city to gather data from intersections, water usage, water height levels, temperatures, pollution, and many many other things around the city, This implementation might only annoy a few city planners and other city officials, but if such a system is used in a place where data is more critical such as healthcare or another environment where no data loss is acceptable but due to insufficient understanding journal data is stored where data critical to human life is lost due to not under the correct understand and information about the system in use.
...
Enable better understanding of databases, transaction models, and use cases doing the teaching of database courses where behavior is better explained. This should not in-depth but just to the level of read uncommitted or last-write-wins databases are fine for logging but should be avoided for most systems as they have much-unintended behavior, or at least theses systems should be below other layers that ensure that the data is persistent before a commit is accepted.

... should maybe be defined as future work.

Whats next, Defining a standard for testing and certifying database systems towards transactional model that allows the industry to standardize the labeling and allows users and developers to choose the current system for their use case and avoid the headaches of phantom behavior.

This could be done via automated testing and verification of a given system in it is different transactional configurations.
...
I will write deeper about designing, implementing, testing, executing, and analyzing the results. However, this does not mean that the learning and understanding phase of the project was any smaller—quite the contrary. Learning two new programming languages that I have never interacted with before is not easy; adding in the complexity of writing code for a cloud framework that I had no prior experience working on did not help either. Spicing it up with outdated documentation, incompatibility between versions of the framework, and lack of debugging tools and broken tools meant that a lot was done via eyes blind brute-forcing solutions to many issues due to lacking features on the Linux version of the cluster.\\
 
 
Next, Defining a standard for testing and certifying database systems towards the transactional model allows the industry to standardize the labeling and allows users and developers to choose the current system for their use-case and avoid the headaches of phantom behavior.

This could be done via automated testing and verification of a given system in its different transactional configurations.
