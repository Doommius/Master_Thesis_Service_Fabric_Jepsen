...
Repeatable Read closely resembles a Serializable system, however in RR we allow phantoms, this meaning that objects are stable once read, but that if we preform the same read within the a given transaction that the predicate itself might not be stable. it is formally defined in ANSI SQL 1999 that specifies RR as as read committed but disallowing Non-repeatable read

\textcolor{red}{TODO Make Diagram}
transaction containing 2 reads of the same data. while a concurrent operation preforms a write to the same data.
