...
\begin{document}
    ...
    %Probably not what you want. Maybe reformulate how to guarantee consistency and ... on systems that are distributed all over the world
    Is it even possible to get both consistency and availability, and is it needed in most use cases?\\
    \vspace{5mm}
...
Here you should write what the thesis is about. In this thesis we .... We show that  ... (1 paragraph summary of results)
...
Structure of the thesis.
    ...
    In Chapter I will introduce Jepsen, a Clojure framework\cite{jepsonio} developed by K. Kingsbury that allows for testing distributed systems. This is done by building a directed serialization graph (DSG) via querying the target system with carefully chosen queries for traceability and recoverability.  \\
    \vspace{5mm}
...
%TODO write section on Dynamo
    ...
    The goal of the thesis was to attempt to preform a 
    The goal is to test the concurrency properties of the ... by using
    Jepsen test on Service Fabric with it's reliable collection as the data store feature that powers a lot of the services being the target of this endeavor.
    
    I miss here a phrase saying that "In this chapter we will present ..." 
...
Consider moving this into conclusions
    ...
    The Design of the services required for preforming the Jepsen test were designed on the fly
    on the fly is the wrong term - consider removing this sentence
    as no prior experience was had with Service Fabric, Jepsen, C\# or Clojure.

    \subsection{Initial Design}

    The initial design of the testing suite contained two applications: a stateful service Fabric application and a Clojure application running the Jepsen test.

    \subsubsection{Service Fabric Stateful Service}

    The Service Fabric stateful service should contain the desired operation that our Jepsen test can interact with, the first designed endpoints 
    
    what are endpoints? Interfaces? Microservice exposing something? Explain better
    were insert/add, Update, Check and set, delete and delete all on a reliable directory class. It would be implemented using C\# as the lack of documentation for java made this this seem easier in the beginning and due to all documentation being in C\#. The service would expose these end points via a web endpoint and kestrel
    citation
    was selected for this as it was a native provided already integrated into SF and was the one most examples used and was therefore deemed the safest choice to proceed with.\\
    \vspace{5mm}
...
Additional modules for the partition-random-halves of the network to attempt to induce a brain split
\begin{lstlisting}
...
Generator that generates the input for the reliable Directory reliable collection.
\begin{lstlisting}
...
(defn ^String url
  ...
  ([client uri]
   (str (base-url) "/" uri))
  ([client uri key-seq]
   (str (base-url) "/" uri "/" key-seq))
  ([client uri key value]
   (str (base-url) "/" uri "/" key "/" value))
  ([client uri key value1 value2]
   (str (base-url) "/" uri "/" key "/" value1 "/" value2)))
\end{lstlisting}  
...
\chapter{Results}
    ...
    The Results of the test show that While Jepsen test preforms fine for single node and partition workloads in terms of Serializability there are some linearizability issues. This however all falls to the ground rather quickly as higher and more complex workloads cause exponential deadlocks and aborts and due to the design of the entire system multipartion workloads that allow for scaling doesn't allow for any consistency which cause a great range of issues where the entirely process of consistency handling lies with the developer. and which requires the developer to write an entire DBMS system on top of the reliable collections as they only provide reliable storage of singular objects once committed.
    
    
    The first telling part of the issues with Reliable collections showed up when doing the development where the complexity required to test service fabric as a multiple agent system grew way out of scale compared to what is with time both the time frame and grasp of this project. A entire DBMS system on top of service fabric would be needed and here it can be argued that you're simply building a reliable system onto of something unreliable. While Service Fabric does appear to have use full use cases it does however not pose well as a transnational data store.
    
    
    The first results from the tested showed much promise for Service Fabric as we were simply testing a linearizable workload where it was simply atomic operations that did not require any complex locking structure. However there was some performance limitation where the API was "Only" able to handle 20k Request per second before reaching a bottle neck and dropping packages due to the request overflowing. which shows there might be some under laying performance issues as well compared to using a framework like Flask or FastAPI and a regular datastore such as SQL, or a key value store that often preforms even better.
...
\end{lstlisting}
    ...
    The results from the first rounds of test showed that the designed testing framework didn't test the system to it's limits and didn't allow for in depth testing and checking via directed serializations graphs so a more complex and in depth testing suite had to be designed and implemented to show faults and issues in the inner workings of the reliable collections. From these tests some interesting results started poping up, The more complex workload didn't consist of simpler and naive transactions like the earlier but also testing handling, conflict handling, transactions with dependencies on itself and more. This brought a few issues with regards to deadlocks out into the light.
    
    
    \subsection{Rejected transactions}
    The first more complex test run on the Service Fabric cluster was a multiple transaction used transaction with 1-10 operations per transaction, this cases a abort rate of 89.5 \%. This was caused by heavy deadlocking a cascading aborts due to timeouts and and circular locking issues. 
    This initially was questioned if this was due to network issues within Azure or latency issues between the nodes. To eliminate the risk of this being the culprit a Proximity group was established. This did however now resolve anything. This brought me back to an earlier belief that SF reliable collections are a distributed Atomic datatype where you preform a singular operation er transaction or lock the entire datastore to preform writes. this does mean that for the transaction that make it though the deadlocking issues SF is serializable which is more than the Reliable read that is presented from Microsoft, but this is the case for all atomic datatypes, but here it comes with built it network (read only) replication. 
    

The full results for the initail test are below.
\begin{lstlisting}
    :stats {:valid? true,
         :count 13086,
         :ok-count 1365,
...
%TODO Write Violations of promised consistency level
