...
\subsection{Database Automation}
 The Database or db module handles starting, stopping, and interaction with the database program or service itself. Its main job is to spin up, prepare and configure the test target. This is not be required for test, but it is best practice to do this as it allows for better repeatably of the tests. A just as important feature handled by this module is interacting with the service once it is running such as killing workers, causing crashes, or removing the entire node from the cluster.
...
The generator's task is to generate the test map, which includes transactions and faults. It is also important that the generator generates transactions in a manner that allows the checker to defer an ordering to allow for correctness checking.

\subsection{Correctness checker}
The checker module checks for anomalies in the results. This can be a host of symptoms, out-of-order operations, violation of isolation or corrupt data. This can either be done by writing custom checkers or using some of the functionality built into Jepsen.
...
Jepsen supports quite a few checking modules, but the most powerful checker is Elle, inferring isolation anomalies from Experimental Observations. \\
\vspace{5mm}
...
These things can be manifested in multiple ways; some cases are, as mentioned earlier in the paper, accepted as eventually consistency and performance are valued higher, or some applications were losing some data points are acceptable to allow for higher write throughput.\\
\vspace{5mm}
...
A test was done on Cassandra in 2013 that showed some interesting behavior, with interesting names such as doomstones, and that during conflicts a lexicographically bigger value is winning. such that separate parts of separate transactions might end up in the database. .\\
\vspace{5mm}
...
\begin{itemize}
    \item During a single key heavy write operations Losing 28\% of committed data and fails linearizable by any definition.
    \item During Write conflicts roughly 1 in 200 rows end up being corrupt due to poor handling time time where milliseconds are used with 3 zeros at the end instead of microseconds.
    \item 
