...
Snapshot isolation Behaves differently to any of the systems, it can be consider more as a try: commit() catch: abort(), we preform our transaction in a independent branch and commit/merge to the database on commit, if there are any conflicts with a already committed transaction we abort.
