...
\chapter{Database Consistency testing Possily rename to Jepsen}
...
\subsubsection{Service Fabric Statefull Service}
...
This implementation contains 2 main codebases. The c# dotnet web service that acts as a restAPI to the reliable collections using 3 controllers for the 2 default data structures implemented in the framwork, and a Clojure codebase that contains the test suite as well as the API connector that handles parsing, connections and connections faults.
...
All traffic recides within the cluster and the load balancer is only there for testing of the application and check if all endpoints work as exspected.

\paragraph*{Node Sizes}

The initial node sizses were 2 cpu cores and 8 gbs of ram alog with 20 gb local storage, as this was thought be enough to handle the workload of the test. This was later revised as the nodes had issues with diskspace and often took a long time to setup and get ready for any given deployment, this was resolved by upgrding the size of the nodes to 4vcpu, 16gb ram and 80gb local storage per node. This configuration resulted in a much more velbehaved cluster. and this node size was kept for the reminder af the test. However the jepsenhost is slightly underdimentioned compared to the workload it handles when computing on the result of the test. This is primary due to OOM errors that could be resolve with wither lowering the search space of the test or increasing the memory of the node itself.

\section{Services}
as mentioned ealier running these tests required multiple components, The stateful service that hosts the reliable collections from SF Reliability subsystem, a restless frontend that provides this data to clients via restapi, a driver that handles the connection to this api and finally the jepsen test suite itself consisting of both test suite and checker of the transactions and fault induser of the end system.

\subsection{SF services}
Using the 2 services approch as recommended from Microsoft provides multiple benefits but also a few negatives. the prime benefit it abstacting from the backend and adding a layer of validation, routing, and protection of the backend service. as well as greatly reducing the time that connections are held open to due RR times as all connections to the datastore is kept withi the cluster. 
\subsubsection{Stateful Reliable collection service}

The stateful sevices provides 3 seperate controllers that each provides one of the built in reliable colllection types provided by service fabric. Reliable dictionary, reliable queue, and reliable concurrent queue. Fault handling is also handled at this stage where any fault stemming from the transacion issues is returned to the client via http codes. eg node not primary, acccepted, result not found.. etc. 

The forend service simply provides these same apis but with wrappes and rotuign in case of a partioned design. This layer could also be used to built addional features but this was not the case as only the bare transactions supported by the reliable collections layer was needed.

\subsection{Jepsen test suite}
The jepsen test suite contains many layers and two main components a connector that conects to the service fabric services, and the test suite itself.

The drive is the simplest service that is based on a raft api that was completely rewritten to interact with the SF API along with a custom fault handler that is able to parse both the result that the reliabl collections api provides along with with the http codes in case something doesn't behave as intended.

The jepsen test component contains 6 classes. a runner, a db class that prepares for the test, a client that interacts with the driver, nemesis that induces faults in the cluster and 3 workloads, queue, cqueue, and dict. testing the 3 seperate types provided by service fabric.

The runner type is the main class that handles defaults, cli input and test selection along with code that handles common behavoir between the test. nemesis, db, along with other features that are mutual for all services that run in serice fabric.
...
the nemesis class that contains the functions that kill, partision or otherwise mess and attempt to break the cluster.

the 3 workloads.

These each contain client for the test defining what endpoint to reach out to, a generator generating a seweunce of transactions to preform on te database, and a checker that checks if our datastored bahaved as exspecetd.

all types are exspecetd to follow the repeatable-read consistentsy model. where the concurrent queue does best effort to meet the fifo order of queued items compared the the normal queue following a fifo ordering.

\subsection{Implementation of the initail design.}


The initial designed first targeted the reliable collection dict datatype. and presented a few issues with the design and implementation details that was not considered. The biggest hurdle here was developing code for the service fabric linux cluster due to a non feature paraty between it and the windows cluster which caused non favable conditions such as no debugging, binary dump files that weren't readable by any tool available, dns service missing which required a lot of endpoints to be hardcoded which meant only one active service per node. eg we can only have 1 service running per node which would greatly reduce veritcal scaling of the partions which would greatly increase the number of partions required to handling greater workloads. this was resolved by designing a 3rd service that simply provides the API directly from the stateful service as the transactions are going to behave the same on a single partion as with 500 partions. this 3rd service was confiured such that replicas were active so they could be queried for read only operations which the primary was in charge of read operations such that all write operations were routed to the leader node. where all reads were spread across the 5 nodes in the cluster. 

\subsubsection{non feature parity between windows and linux in Service fabric}
For developing the above services a development enveroiment had to be configure, test before any development could take place,

For service fabric this resolved around installing required frameworks, development kit and getting everythig running, this turned out to be exponentionally more difficult then initially assumed as service fabric was initially designed for windows with the linux version not having the same features and not being interchanable with the windows versions.

The initial development was done aiming a local service fabric cluster with 5 nodes running linux on my desktop. development here was pretty smooth after i got everything working. 

Issues here resolved mainly around all exsamples and documentation used outdated examples, code and modules, and the skeleton code generated had these same issues. the first issue here to tackle was the outdated frameworks that had to be updated to generate a accuate result of the consistentcy in the service. using old frameworks and liberies isn't lining the odds towards service fabric in any way and if not does the opposite. this process of updating the libreaies causes a lot of broken functinality that had to be resolved before any code could be executed but the majority of this intial work was done on the windows cluster, which meant that features such as debugging, codedumps and errors could be used to debug and resolve any issues in the code and setup. here a usable solution was designed. The code was now ready to push to azure for the real deployment of the project. here however things didn't work either, and no feedback on why it didn't work was given. other than an entrypoint error in the service fabric manager which continuesly tried to build, test and run the application until drives filled up and the cluster crashed.

\includegraphics{images/cluster_died.png}

Doing this process this occured many times and required a redeploy of the cluster. what was filling up the storage on the cluster was all the failed builds. these were stored. until they were resolved which by default they weren't. but when the cluster filled up it's storage in a matter of minutes as it repeatably tried to build a broken applicaiton instead of retrying a set amount of times before marking the application as broken/failed.

The above issue kept given me issues as the task of deleting these crash reports needs to be done via either the azure cli or the azure portal, where each nodes crash reports have to be deleted one by one, here also my restricted permissions gave me issues as this wasn't something that could be automated via the azure cli.





 this meant that the initial code i used to setup the my local envoriment and that i had used to design, configure and setup things turned out to non a dod. redesigning this for linux turned out to be much much more complicated as many of the features and tools developers rely on to debug and write code wasn't there. For many parts this turned the development cycle into testing on a blackbox system where we only knew the pre compiled code. and the output. due to the multiple layers in the intial implementioned this caused quite a few issues as i had two black boxes. my client talking to the stateless service that requetsed data from the stateful service, which return data, or crashed, and the stateless service either returned the result or an empty result. However due to the mentioned missing features in the linux version the interal routing framework isn't a feature on linux so any conneciton between nodes is done via ip/tcp. which meant that both services were reachable. 
